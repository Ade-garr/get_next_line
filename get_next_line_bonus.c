...
/*   Updated: 2020/01/06 16:24:21 by ade-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	left[400][BUFFER_SIZE + 1];
	int			ret;

	if (line == NULL || fd < 0 || BUFFER_SIZE < 1)
		return (-1);
	if (ft_checkleft(left[fd]) == 0)
	{
		ret = ft_fill_line(fd, line, &(left[fd]));
		if (ret == -1 || ret == 0)
			return (ret);
		ft_moveleft(left[fd], left[fd] + ft_position(left[fd]) + 1);
	}
	else
	{
		ret = ft_give_line(line, left[fd]);
		if (ret == -1)
			return (ret);
	}
	return (1);
